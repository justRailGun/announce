...
const CommentSection =  ( { id } :  { id: string } )  => {
  ...
  async function onSubmit(values: z.infer<typeof schema>) {
      ...
      const res =  await fetch(`http://localhost:3000/api/products/comments`, {
        method: 'POST',
        body: JSON.stringify({body : validatedData.data , author : session?.user?.id, product : id}),
        headers: {
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input className="dark:bg-gray-700 placeholder:dark:text-white/80" placeholder="Share Your Opinion" {...field} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input  className="dark:bg-gray-700 placeholder:dark:text-white/80" placeholder="your rate" {...field} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
        ...
        {comments && comments.map((comment) => (
          <div key={comment._id} className="mb-4 p-4 bg-gray-100 rounded-lg">
            <div className="flex items-center justify-between mb-2">
              <span className="font-semibold text-dark-200">{comment.author}</span>
              <div className="flex">
                {[...Array(5)].map((_, i) => (
                  <Star
                    key={i}
